...
- **테스트 우선 원칙 적용**:
  ...
  - 각 기능 구현 직후 즉시 테스트 수행
  - 테스트 실패 시 구현 수정 후 재테스트
  - 모든 테스트 통과 확인 후에만 다음 기능으로 진행
- **테스트 커버리지 요구사항**:
  - API 엔드포인트별 단위 테스트
  - 비즈니스 로직별 테스트 케이스
  - 사용자 플로우 E2E 테스트
  - 에러 핸들링 및 엣지 케이스 테스트
- 각 구현 단계 완료 후 테스트 결과 기록 및 검증
...
- **Task 005: 데이터베이스 및 API 개발** - 우선순위
  ...
  - **필수 테스트 항목**:
    - 각 API 엔드포인트별 Playwright MCP 테스트 작성
    - CRUD 작업 검증 (Create, Read, Update, Delete)
    - 요청/응답 데이터 형식 검증
    - 에러 상태 코드 테스트 (400, 401, 403, 404, 500)
    - 페이지네이션 및 필터링 기능 테스트
...
- **Task 006: 인증 및 권한 시스템 구현**
  ...
  - **필수 테스트 항목**:
    ...
    - 로그인/로그아웃 시나리오 테스트
    - 세션 유지 및 만료 검증
    - 권한별 접근 제어 테스트 (unauthorized access)
    - 토큰 갱신 및 무효화 테스트
    - 보안 취약점 검증 (CSRF, XSS 방지 등)
    - 모든 테스트 통과 후에만 다음 Task로 진행

- **Task 006-1: 핵심 기능 통합 테스트** - 필수
  - Playwright MCP를 사용한 전체 사용자 플로우 테스트
  - API 연동 및 비즈니스 로직 검증
  - 에러 핸들링 및 엣지 케이스 테스트
  - **필수 테스트 시나리오**:
    - 주요 사용자 시나리오별 E2E 테스트
    - 여러 API 호출이 연계된 복합 플로우 테스트
    - 동시성 처리 및 경쟁 조건 테스트
    - 네트워크 오류 시뮬레이션 및 재시도 로직 검증
    - 성능 및 로딩 시간 검증
...
- **Phase 3: 핵심 기능 구현** (테스트 집중 단계)
  - 데이터베이스 연동 및 API 개발
  - 인증/권한 시스템 구현
  - 핵심 비즈니스 로직 구현
  - 더미 데이터를 실제 API로 교체
  - **각 기능 구현 시 Playwright MCP 테스트 필수**:
    - 구현 → 테스트 → 검증 → 다음 기능 순서 엄수
    - API 엔드포인트별 단위 테스트 작성
    - 비즈니스 로직별 테스트 케이스 작성
    - 통합 테스트로 전체 플로우 검증
    - 모든 테스트 통과 없이는 다음 단계로 진행 금지
...
- 측정 가능한 완료 기준 제시
- **API/비즈니스 로직 Task의 경우 반드시 "필수 테스트 항목" 섹션 포함**:
  - 각 기능별 Playwright MCP 테스트 시나리오 명시
  - 정상 케이스 및 에러 케이스 테스트 정의
  - 테스트 통과 기준 명확히 작성
  - 예시: "모든 API 엔드포인트 테스트 100% 통과"
...
- [ ] 모든 API 엔드포인트별 테스트 케이스가 정의되었는가?
- [ ] 각 비즈니스 로직별 테스트 시나리오가 작성되었는가?
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] 에러 핸들링 및 엣지 케이스 테스트가 고려되었는가?
- [ ] Phase 3에 통합 테스트 Task가 별도로 포함되었는가?
- [ ] **구현 → 테스트 → 검증 사이클이 워크플로우에 명시되었는가?**
- [ ] **테스트 통과 없이 다음 단계로 진행 금지가 명확히 표시되었는가?**
- [ ] 각 Task에 "필수 테스트 항목"이 구체적으로 나열되었는가?
- [ ] 보안 관련 테스트(인증, 권한, 취약점)가 포함되었는가?
